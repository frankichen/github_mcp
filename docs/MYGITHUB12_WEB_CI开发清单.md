...
Pre-final-repair historical code-candidate validation：Index `12.0.0-1` Job `0b3ee973-73d5-4e5d-9e7b-3ccc66942e32` completed（288 files，reused 286 / reindexed 2）；Change Impact = 5 changed paths / modules `docs, services` / 61 affected-test candidates / Contract Change 0。full Private CI `0ebc9f8e4e8c40c2` = passed / exit 0；`github-action-service` ruff + compileall passed，full pytest `886 passed in 11.10s`（`--durations=50 --durations-min=0.05` 保留）；`private-ci-agent` ruff + compileall passed，pytest `230 passed in 49.07s`；`private-deploy-agent` pytest `5 passed in 0.08s`。Attestation `c1649e46-c685-4ae5-8831-e8fc3a1e5f08` 仅绑定此前 performance-repair code candidate；Final repair 新 Commit 后该 Index / CI / attestation / Clean evidence 均不再作为当前完成证据。

**Final repair（BLOCKER-017-02 / MAJOR-017-03 / MAJOR-017-04）**：repair code candidate `3faa64d973d01f3a3e7ac496c61f635c97e0a1b0` / Tree `318c2001953f74d908de5279e8dadb83096e85a4`。

- `BLOCKER-017-02 secret redaction repair`：`_SENSITIVE_OPTION_RE` 增加真实 option boundary `(?<![\w.-])`，不再从普通 assignment identifier 内部的 `-secret` 起匹配；CLI matcher 同时覆盖空格、`=`、单双引号 value。payload assignment 不再执行宽泛 `_SENSITIVE_ASSIGNMENT_RE` / `_QUOTED_SENSITIVE_RE`，改为 `:` / `=` separator 驱动的局部 key/value 单向扫描，覆盖 unquoted/quoted key、unquoted/quoted value、`.` / `_` / `-` key 变体。PEM、Authorization、headers、URL credentials、known token formats、api/access/private keys、credential、password/passwd、secret 等既有层继续保留。Deterministic matrix 覆盖 `token=SECRET`、`token = SECRET`、`token: SECRET`、`client_secret/client-secret/client.secret`、quoted value、quoted key、`--token` space/equal/quoted、`--client-secret`；每例都断言原 secret value 不存在且 `[REDACTED]` 存在，并单独断言 `client-secret = SECRET -> client-secret = [REDACTED]`，防止 regex ordering corruption。large quoted secret near tail、large spaced sensitive assignment near tail 与 large ordinary payload bypass regression 同时保留。
- `MAJOR-017-03 Windows punctuation compatibility`：location scanner 在 `,`、`;`、`[`、`<` 等外围标点后仅在后续明确为 absolute/Windows path 时建立新 candidate boundary，保留 Windows drive prefix；`C:\src\a.py:7:2` 在 comma / semicolon / square bracket / angle bracket 四种形态均期望 `file=C:\src\a.py, line=7, column=2`。
- `MAJOR-017-04 URL same-token compatibility`：移除 token 级 `token_has_url` veto，改为 scanner 内的 `url_active` candidate 状态；URL candidate 被拒绝不再导致同一 token 后续显式 Windows/Unix candidate 一并失效。regression 覆盖 URL 后 comma / semicolon / whitespace + `C:\src\tail.py:7:2`，以及 `https://example.test:443/path,/tmp/tail.py:321:9 -> /tmp/tail.py:321:9`；明确不恢复旧 `/path,/tmp/tail.py` 拼接 false-positive。

Differential compatibility corpus 使用 base `parse_failure_location` matcher 作为 bounded comparison oracle，覆盖 Unix、Windows、leading/trailing punctuation、URL、multiple candidates、same-token candidates、`file:line`、`file:line:column`、invalid numbers、parentheses、whitespace；差异显式分类为 `compatible`、`compatibility_restored`、`bug_fix`、`intended_tightening`。目标是保留合法 extraction，并拒绝 base 的 URL-path 拼接 false-positive，而不是逐字复制旧错误行为。

Final-repair isolated exact-function benchmark（从该 candidate 的 production 实现回读后复刻，仅用于复杂度趋势，不冒充 exact-head repo CI）：`redact_text` ordinary 5/10/20/80/320 KiB = `0.973/1.766/3.494/17.431/57.415 ms`；quoted-secret-near-tail = `1.331/2.858/5.725/22.587/102.961 ms`。`parse_failure_location` no-location 8/16/32/128/512 KiB = `2.095/4.499/11.327/34.784/132.966 ms`。5/10/20 KiB redaction 约随输入翻倍接近 2x；20->80 KiB 4x 输入 ordinary 约 4.99x、secret-tail 约 3.95x；32->128 KiB parse 4x 输入约 3.07x、128->512 KiB 4x 输入约 3.82x，未回到 repair 前 near-quadratic 每翻倍接近 4x 的增长。isolated semantic smoke 同时确认原六个泄漏例均不保留 secret，Windows punctuation、URL+Windows tail、URL+Unix tail 均得到期望结果。

Final-repair code-candidate exact-head validation：Index `12.0.0-1` Job `88e090f8-16ca-474e-abae-d4ea7e909af6` completed（288 files，reused 286 / reindexed 2）；Change Context 已重建；Change Impact = 5 changed paths / modules `docs, services` / 61 affected-test candidates；Contract Change Detection = 0。full Private CI `014f4c59f7cc4eaf` = passed / exit 0，source mirror exact HEAD `3faa64d973d01f3a3e7ac496c61f635c97e0a1b0` / Tree `318c2001953f74d908de5279e8dadb83096e85a4`；`github-action-service` ruff + compileall passed、full pytest `927 passed in 10.04s`；`private-ci-agent` ruff + compileall passed、pytest `230 passed in 47.78s`；`private-deploy-agent` ruff + compileall passed、pytest `5 passed in 0.08s`。Attestation `b7f99b23-20b5-4709-8401-c52e424a041f` 绑定该 code candidate。固定 `repo-fast-check` preflight 返回 `fast_check_makefile_missing`，因此没有伪造 targeted fast-CI 结果；两个最高 affected tests `test_development_failure_pack.py` / `test_web_ci_get_snapshot.py` 已包含在 exact-head full CI 的 controller full pytest 中。此处仍是 docs commit 前 code evidence；本文档 Commit 产生新 HEAD 后，必须再次重建最终 Index / Change Context / Impact / Contract / Affected Tests / full Private CI / attestation / GitHub Checks / Clean 2/2。
