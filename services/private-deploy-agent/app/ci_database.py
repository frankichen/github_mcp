...
def create_or_get_job(
    ...
    db.execute(
        """INSERT INTO ci_jobs (job_id, idempotency_key, repository, branch, commit_sha, profile, priority, status, timeout_seconds, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, 'queued', ?, ?)""",
        (job_id, idem_key, repository, branch, commit_sha, profile, priority, timeout_seconds, ts),
    )
...
def lease_job(worker_id: str, supported_profiles: list[str], max_concurrent: int) -> Optional[dict]:
    ...
    try:
        ...
        # Fair scheduling:
        # 1. Get queued jobs ordered by priority (lower=higher)
        ...
        rows = db.execute(
            f"""SELECT j.*, COALESCE(rqs.last_dispatched_at, 0) as repo_last_dispatched
               ...
               ORDER BY j.priority ASC, repo_last_dispatched ASC, j.created_at ASC
               LIMIT 1""",
            supported_profiles,
        ).fetchone()
...
def finish_step(step_id: int, status: str, exit_code: Optional[int] = None, log_end_offset: Optional[int] = None):
    db = _get_db()
    ts = now_ts()
    row = db.execute("SELECT started_at FROM ci_job_steps WHERE id = ?", (step_id,)).fetchone()
