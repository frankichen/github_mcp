...
def runtime_build_sha() -> str:
    configured = os.environ.get("MYGITHUB10_BUILD_SHA", "").strip()
    if BUILD_SHA_RE.fullmatch(configured):
        return configured
    runtime_mode = os.environ.get("MYGITHUB10_RUNTIME_MODE", "development")
    if runtime_mode != "development":
        raise RuntimeError("MYGITHUB10_BUILD_SHA must be a full lowercase 40-character Git commit SHA")
...
def validate_runtime_metadata() -> None:
    runtime_build_sha()
    configured_version = os.environ.get("MYGITHUB10_VERSION", "").strip()
    if configured_version and configured_version != SERVICE_VERSION:
        raise RuntimeError(
            f"MYGITHUB10_VERSION must match service version {SERVICE_VERSION}"
        )
