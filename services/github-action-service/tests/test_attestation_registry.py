...
def _job():
    return {"status": "passed", "exit_code": 0, "superseded_by_job_id": None,
            ...
            "base_sha": "b" * 40, "changed_files": ["a.go"], "summary": {"git_tree_sha": "t" * 40,
            "image_digest": "sha256:image", "go_version": "go1.25", "node_version": "node22", "npm_version": "11",
            "evidence": {"base_sha": "b" * 40, "changed_files": ["a.go"], "go_sum_sha256": "g", "admin_lock_sha256": "a", "console_lock_sha256": "c", "test_config_sha256": "x"}}}
...
def test_artifact_validation_requires_real_bundle_files(tmp_path, monkeypatch):
    ...
    result = registry.validate_artifact(item["artifact_id"], repository="owner/repo", branch="main", commit_sha="c" * 40, tree_sha="t" * 40, private_ci_job_id="job")
    assert result["ok"] is False and result["error_code"] in {"ARTIFACT_ARCHIVE_INVALID", "ARTIFACT_UNSAFE_ARCHIVE_ENTRY"}
