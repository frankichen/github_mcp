...
PRODUCT_CONTRACT_VERSION_RE = re.compile(
    r"^docs/product-contracts/(?P<domain>[^/]+)/(?P<date>\d{4}-\d{2}-\d{2})-(?P<revision>r[0-9A-Za-z._]+)-(?P<slug>[^/]+)\.md$"
)
PRODUCT_CONTRACT_REQUIRED_METADATA = ("contract_id", "contract_revision", "approved_source", "supersedes")
PRODUCT_CONTRACT_IMPLEMENTATION_PREFIXES = (
    "cmd/", "internal/", "api/openapi/", "db/migrations/",
    "h5/", "tests/", "tools/", "scripts/",
)
PRODUCT_CONTRACT_FILENAME_TOKENS = ("需求", "契约", "协议", "规范", "产品规则")


def _contract_git(source_dir: str, *args: str):
...
def _contract_git_text(source_dir: str, revision: str, path: str) -> str:
    result = _contract_git(source_dir, "show", f"{revision}:{path}")
    return result.stdout if result.returncode == 0 else ""


def _contract_changed_entries(source_dir: str, base_sha: str, head_sha: str):
    result = _contract_git(source_dir, "diff", "--name-status", "--find-renames", f"{base_sha}...{head_sha}")
    ...
    for line in result.stdout.splitlines():
        ...
        if status.startswith("R") and len(parts) >= 3:
            entries.extend((("D", parts[1]), ("A", parts[2])))
...
def _is_frozen_product_contract(path: str, text: str) -> bool:
    ...
    if not any(token in filename for token in PRODUCT_CONTRACT_FILENAME_TOKENS):
        return False
    return bool(re.search(
        r"(?mi)^\s*>?\s*(?:文档)?状态\s*[：:]\s*.*(?:已确认|已冻结|冻结|已定稿|产品依据|开发和验收依据|开发与验收依据)",
        header,
    ))


def _product_contract_metadata(text: str, key: str) -> str:
    match = re.search(rf"(?mi)^\s*{re.escape(key)}\s*:\s*(.+?)\s*$", text)
    return match.group(1).strip() if match else ""


def _product_contract_error(code: str, path: str, message: str):
    return {"code": code, "path": path, "message": message}


def _validate_new_product_contract(path: str, text: str):
    findings = []
    match = PRODUCT_CONTRACT_VERSION_RE.match(path)
    if not match:
        return [_product_contract_error(
            "PRODUCT_CONTRACT_VERSION_PATH_REQUIRED", path,
            "new frozen contract must use docs/product-contracts/<domain>/<YYYY-MM-DD>-rN-<slug>.md",
        )]
    missing = [key for key in PRODUCT_CONTRACT_REQUIRED_METADATA if not _product_contract_metadata(text, key)]
    if _product_contract_metadata(text, "contract_status").lower() != "frozen":
        missing.append("contract_status: frozen")
    if missing:
        findings.append(_product_contract_error(
            "PRODUCT_CONTRACT_METADATA_INVALID", path, "missing/invalid: " + ", ".join(missing)
        ))
    revision = _product_contract_metadata(text, "contract_revision")
    if revision and revision != match.group("revision"):
        findings.append(_product_contract_error(
            "PRODUCT_CONTRACT_REVISION_MISMATCH", path,
            f"filename revision {match.group('revision')} != contract_revision {revision}",
        ))
    if _product_contract_metadata(text, "approved_source").lower() in {"tbd", "todo", "unknown", "待确认"}:
        findings.append(_product_contract_error(
            "PRODUCT_CONTRACT_METADATA_INVALID", path,
            "approved_source must reference a real product decision/source",
        ))
    return findings


def verify_product_contract_integrity(repository: str, source_dir: str, base_sha: str, head_sha: str):
    """Trusted host-side guard: product contract history may not be rewritten by the tested PR."""
    if not os.path.isfile(os.path.join(source_dir, PRODUCT_CONTRACT_README)):
        return {"ok": True, "applicable": False, "errors": [], "checked_entries": 0}
    errors = []
    if not PRODUCT_CONTRACT_SHA_RE.fullmatch(base_sha or ""):
        errors.append(_product_contract_error("CONTRACT_BASE_SHA_REQUIRED", PRODUCT_CONTRACT_DIR, "exact base SHA is required"))
        return {"ok": False, "applicable": True, "errors": errors, "checked_entries": 0}
    if not PRODUCT_CONTRACT_SHA_RE.fullmatch(head_sha or ""):
        errors.append(_product_contract_error("CONTRACT_HEAD_SHA_REQUIRED", PRODUCT_CONTRACT_DIR, "exact head SHA is required"))
        return {"ok": False, "applicable": True, "errors": errors, "checked_entries": 0}
    actual_head = _contract_git(source_dir, "rev-parse", "HEAD")
    if actual_head.returncode != 0 or actual_head.stdout.strip() != head_sha:
        errors.append(_product_contract_error("CONTRACT_SOURCE_HEAD_MISMATCH", PRODUCT_CONTRACT_DIR, "source checkout does not match requested head SHA"))
        return {"ok": False, "applicable": True, "errors": errors, "checked_entries": 0}
    if _contract_git(source_dir, "cat-file", "-e", f"{base_sha}^{{commit}}").returncode != 0:
        errors.append(_product_contract_error("CONTRACT_BASE_COMMIT_UNAVAILABLE", PRODUCT_CONTRACT_DIR, "base commit is unavailable in trusted source mirror"))
        return {"ok": False, "applicable": True, "errors": errors, "checked_entries": 0}
    if _contract_git(source_dir, "merge-base", "--is-ancestor", base_sha, head_sha).returncode != 0:
        errors.append(_product_contract_error("CONTRACT_BASE_NOT_ANCESTOR", PRODUCT_CONTRACT_DIR, "base SHA is not an ancestor of head SHA"))
        return {"ok": False, "applicable": True, "errors": errors, "checked_entries": 0}
    entries, diff_error = _contract_changed_entries(source_dir, base_sha, head_sha)
    if diff_error:
        errors.append(_product_contract_error("CONTRACT_DIFF_UNAVAILABLE", PRODUCT_CONTRACT_DIR, diff_error))
        return {"ok": False, "applicable": True, "errors": errors, "checked_entries": 0}
    new_contracts = set()
    for status, path in entries:
        previous_text = _contract_git_text(source_dir, base_sha, path)
        current_text = "" if status == "D" else _contract_git_text(source_dir, head_sha, path)
        if previous_text and _is_frozen_product_contract(path, previous_text) and current_text != previous_text:
            errors.append(_product_contract_error(
                "HISTORICAL_CONTRACT_MUTATION", path,
                "frozen product contract is immutable; create a new versioned revision instead",
            ))
        if status == "A" and _is_frozen_product_contract(path, current_text):
            if not path.startswith(PRODUCT_CONTRACT_DIR):
                errors.append(_product_contract_error(
                    "PRODUCT_CONTRACT_VERSION_PATH_REQUIRED", path,
                    "new frozen product contracts must use the versioned docs/product-contracts path",
                ))
            elif path != PRODUCT_CONTRACT_README:
                new_contracts.add(path)
                errors.extend(_validate_new_product_contract(path, current_text))
        elif path.startswith(PRODUCT_CONTRACT_DIR) and path != PRODUCT_CONTRACT_README:
            if status == "D":
                errors.append(_product_contract_error("HISTORICAL_CONTRACT_MUTATION", path, "versioned product contract cannot be deleted"))
            elif status in {"A", "M"}:
                new_contracts.add(path)
                errors.extend(_validate_new_product_contract(path, current_text))
    implementation_paths = sorted({
        path for status, path in entries
        if status in {"A", "M", "D"} and path.startswith(PRODUCT_CONTRACT_IMPLEMENTATION_PREFIXES)
    })
    if new_contracts and implementation_paths:
        errors.append(_product_contract_error(
            "CONTRACT_AND_IMPLEMENTATION_MIXED", sorted(new_contracts)[0],
            "contract-only revision cannot change implementation/OpenAPI/Migration/tests/callers; first implementation path: " + implementation_paths[0],
        ))
    return {
        "ok": not errors, "applicable": True, "errors": errors,
        "checked_entries": len(entries), "new_contract_revisions": sorted(new_contracts),
    }
        ...
        if job.profile in {"repo-fast-check", "repo-auto-check"}:
            ...
            if contract_gate.get("applicable"):
                self.log_manager.upload(
                    job_id, f"[contract-integrity] checked_entries={contract_gate.get('checked_entries', 0)}\n"
                )
                if not contract_gate.get("ok"):
                    for finding in contract_gate.get("errors", []):
                        self.log_manager.upload(
                            job_id,
                            f"[contract-integrity] {finding.get('code')} {finding.get('path')}: {finding.get('message')}\n",
                        )
                    summary = {
                        "status": "failed", "exit_code": 2, "steps": [],
                        "contract_integrity": contract_gate,
                    }
                    self.log_manager.flush(job_id)
                    self.client.finish_job(
                        job_id, 2, "failed", summary=summary,
    ...
    def _execute_workspace(self, job: Job, workspace: dict) -> dict:
        ...
        for check in commands.get("check", []):
            ...
            if check["name"] == "ai-integrity":
                ...
                if getattr(job, "contract_integrity_attested", False):
