...
MyGithut12 源码当前版本为 `12.6.3`；生产运行版本必须以 `get_mygithub_capabilities` 的实时结果为准。所有 Commit 类写入在返回成功前都必须完成 GitHub fresh read-back：目标 branch HEAD、新 Commit、Commit Tree 和 changed-path Blob 必须与本次写入严格一致；只有 durable verify 通过后才允许推进 Workspace CAS 与 `success_verified` 幂等状态。AI 日常生成 UTF-8 文本文件的唯一推荐写入入口是 `put_generated_files`：调用方只提供 repository、branch、expected HEAD、`files[{path,content}]`、提交信息、dry-run 和可选幂等 key，服务端会自动绑定该分支唯一有效的 Workspace/Development Session，并在内部完成 revision CAS、旧 blob 读取、路径校验、hash、chunk、staging、原子 Commit、read-back 以及 Workspace/Session 写后推进。V1 不支持 binary、delete 或 bundle，超限 payload 直接返回 `PAYLOAD_TOO_LARGE`；旧高层写入和低层 upload 工具仅保留兼容，不作为 AI 日常开发路径，默认生产 Schema 会隐藏这些兼容写入工具。
...
MyGithut12 现在区分 canonical production Schema 与 compatibility registration：兼容层注册 173 个工具，生产默认 Schema 向 AI 暴露 163 个工具，并隐藏 Manifest 声明的 10 个 deprecated/compatibility 工具；旧 handler 保留兼容调用能力。`put_generated_files`、`resume_development_task` 与 `recover_drifted_development_task` 均属于 canonical Schema。`get_mygithub_capabilities` 会基于当前实际可见 Schema 返回 `tool_schema_sha256`、`schema_generation_id`、可见工具数和兼容工具数，从而可以直接识别 Connector Schema 是否同步。`resume_development_workspace` 是 expired Workspace 的唯一显式恢复入口；drifted Workspace 则只能走受保护的 `recover_drifted_development_task`。只读 `plan_private_ci_job` 会在启动 CI 前按准确 Commit、仓库固定 policy、Manifest/workspace 和固定入口判断 `applicable/reason/detected_stacks/selected_profiles/workspaces`，但不会排队执行 CI。基础设施自部署工具只接受固定仓库、固定环境、固定 scope、exact main、repo-auto-check 和 current-build CAS，不接受 host、shell、script、rollback 或 failure-mode 参数。
...
MyGithut12 `12.6.3` 源码的 compatibility registration 为 173 个工具，canonical production Schema 为 163 个可见工具；生产 Schema 身份必须以运行时 capability 为准。本版继续让 `put_generated_files` 自动绑定活动 Workspace/Development Session，在保持轻量 schema 的同时执行协作 revision CAS；新增 `recover_drifted_development_task` 只推进经过验证的控制面身份，不改变 Git refs。旧写入工具和低层 upload 工具继续保留兼容调用能力，但默认从 AI 窗口可见 Schema 中隐藏。Repository Index 数据格式没有改变，因此 `repository_index_version` 继续为 `12.0.0-1`。
