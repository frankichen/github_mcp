...
BASE-SYNC-RECOVERY-01 新增 `recover_base_synced_development_task`：与原 same-base recovery 分离，调用方必须显式提供 old pinned base、new live base、old Session HEAD 与 current HEAD/Tree。服务端验证 base/task 双 ancestry，把 `old_base -> new_base` 的 Base Delta 与 `new_base -> current_head` 的 Task Delta 分开；Base Delta 与旧 Task Delta 有路径交集时 fail-stop，Workspace scope 只校验新 Base 下的 Task Delta，并要求无冲突路径场景下同步前后的 Task path set 一致。成功时 Workspace/Session 的 base + HEAD/Tree 在同一事务中推进并清除旧 Index/CI/attestation evidence；随后以 new base 请求 current HEAD Index。`resume_development_task` 会在 pinned base 已落后 current base 时返回该显式 recovery action。managed PR merge lifecycle 现在由共享 finalization helper 收敛：`finalize_development_task(action="merge")` 与公开 `merge_github_pull_request` 在 GitHub merge 成功后都必须 exact 证明 PR/head/base/merge_commit evidence，随后同一事务把 Session 标记为 `merged`、Workspace 标记为 `closed` 并释放 lease/index pin；公开 merge 遇到 unmanaged PR 返回 `managed_finalization=not_applicable`，遇到已完成 GitHub merge 但 control-plane finalization 失败时返回 `merge_completed=true` 与 `recovery_required=true`。base-sync recovery 对历史遗留 active Workspace 只有在其 Session 已 `merged` 且 HEAD 可精确证明已进入 new base 时才忽略 overlap，不关闭全局 overlap 门禁。
